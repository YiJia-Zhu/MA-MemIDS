...
══════════════════════════════════════════
  ...
  社区规则库 R_init
       │
       ▼
  【显式特征整理 + Reference Resolver】
  （吸收 reference:* 里的结构化 ID，并对 URL 做
   best-effort 网页解析 / 缓存 / 失败降级）
       │
       ▼
...
Step 0：显式特征清单整理
  feature_inventory =
    ...
    + rule_references（reference:* 原始字段）
    + line_features

Step 0.5：Reference Resolver（仅规则初始化侧）
  ├─ 对 `reference:cve,...`、显式 `Txxxx` 等结构化线索直接规范化
  ├─ 对 `reference:url,...` 单独抓取页面并解析为结构化证据
  ├─ init 阶段会先对全部规则的 `reference:*` 做一轮有界并发预抓取（默认 4 workers）
  ├─ 若 URL 未显式写 `http://` / `https://`，则自动补全，优先尝试 `https://`
  ├─ `ReferenceParse LLM` 只在“文本页 + heuristic 未抽到 CVE/ATT&CK ID + 页面长度达到阈值”时触发
  ├─ 输出：reference_evidence =
  │    {trusted_cve_ids, trusted_tech_ids, trusted_terms,
  │     reference_summary, source_urls, resolved_references}
  └─ 若网页不可访问 / 超时 / 403 / 404 / 解析失败：
       不阻塞 init，自动降级为“结构化 reference + 当前双路检索”

Step 1：Retrieval Planner
  输入：c_i + feature_inventory + reference_evidence
  输出：
    sparse_query
    dense_query
    selected_features
...
Step 4：结果融合（RRF）
  ...
  ├─ 对 reference_evidence 中的可信 CVE / ATT&CK ID 做 exact pin
  ├─ 用 `RRF(k=60)` 统一融合
  └─ 输出：A_enriched = {cve_docs, attack_docs, cti_docs, cve_ids, tech_ids, debug}
```

**双路的互补性**：Sparse 路径擅长命中结构化标识符、payload 关键片段和路径/函数名等强关键词；Dense 路径擅长对齐“攻击意图 + 协议 + payload 信号 + 网络角色”这类语义描述。当前实现不是“让 Dense 直接吃原始流量全文”，而是先由 planner 判断该搜什么，再把语义改写后的 `dense_query` 送入 embedding 检索。若规则本身已有人工维护的 `reference:*`，其产出的可信证据优先级高于普通双路搜索，Hybrid Retrieval 只做补充，不覆盖 reference 证据。
...
Step 1: 规则解析
  ...
  feature_inventory_i = 提取规则中的 content / pcre / msg /
                        protocol / src,dst / CVE / ATT&CK / reference:* 等显式特征

Step 2: Reference Resolver
  输入：feature_inventory_i.rule_references
  输出：
    reference_evidence_i = {
      trusted_cve_ids, trusted_tech_ids,
      trusted_terms, reference_summary, source_urls
    }
  说明：
    - 结构化 reference 直接吸收
    - URL 页面解析失败时不抛异常，直接降级

Step 3: Retrieval Planner
  输入：c_i + feature_inventory_i + reference_evidence_i
  输出：
...
Step 4: 双路知识检索
  ...
  说明：
    - 对 trusted_cve_ids / trusted_tech_ids 做 exact pin
    - 若 reference 页面解析失败，则回退为普通 Hybrid Retrieval
...
**LLM Prompt（ReferenceParse，独立于主 Planner）：**
...
**当前工程实现说明**：代码已切换为“显式特征清单 + Reference Resolver + 检索规划 + 候选召回 + 统一精排打分”。embedding 默认使用 `sentence-transformers/all-MiniLM-L6-v2`（`384` 维），首次运行会自动下载并缓存。知识召回层现为统一的 Hybrid Retrieval：先整理规则/流量的显式 `feature_inventory`；若规则包含 `reference:*`，则先走独立 `Reference Resolver`，将结构化 ID 和网页证据压缩为 `reference_evidence`；再由 Retrieval Planner 生成 `sparse_query` 与 `dense_query`，并额外标记 `selected_features` / `discarded_features`；然后将 ATT&CK STIX 与 CVE 官方 JSON 预处理成标准文档，并分别执行 Sparse Retrieval（SQLite FTS5 / BM25）和 Dense Retrieval（embedding 近邻），最后使用 `RRF(k=60)` 融合两路前 `top5` 候选。对 `reference_evidence` 给出的可信 CVE / ATT&CK ID，检索层会先做 exact pin，再让 Hybrid Retrieval 补充上下文；如果网页抓取失败、403/404、超时、内容过大或解析异常，则自动降级，不阻塞 init。`reference_cache` 默认会自动清理，策略为保留 14 天、最多 2000 个文件、总大小最多 256MB，也可通过环境变量覆盖。Dense 查询不再直接吃整段原始流量，而是使用语义化改写后的检索描述；若环境中可用 `hnswlib`，知识库 Dense 检索会自动加载预构建 HNSW；否则回退为精确 Dense 扫描。最终建边时不再使用“同协议固定权重”或 `cos >= 0.75` 的硬门槛，而是统一使用融合分数，并保留 `exploit_chain / subsume / tactic_group / semantic_similar` 作为关系标签。
...
**PCAP → Agent 输入**：LLM 无法直接消费二进制，工程实现中会先对 PCAP 做**流式采样解析**，只保留结构化摘要（协议、五元组、HTTP 解析、文本型 payload 预览），而不会把整个大文件 body 或 10 万包 payload 全量拼进内存。当前策略包括：

- 最多只采样前 `N` 个数据包（默认 `4096`）
- 只保留主流（primary flow）的前 `M` 字节 payload 预览（默认 `16KB`）
- 对明显的二进制内容、大文件下载 body、附件响应体自动跳过
- 对文本型请求/响应只保留截断后的 payload 预览，并记录 `payload_truncated / binary_payload_skipped / packet_limit_reached`

因此，后续进入 Note 构建与检索规划的是“受控大小的结构化摘要”，不是原始 PCAP 全文。
...
2. 若提供 PCAP：先解析为结构化文本（协议、五元组、HTTP、payload 预览），解析阶段采用流式采样、限包、限 payload，并对二进制/下载 body 自动跳过。

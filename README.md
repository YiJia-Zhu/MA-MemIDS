...
- 阶段二在线：未匹配流量 -> 流量 Note -> Top-k 检索 -> 规则增修
- 沙盒评分：`Score = F2 * P_fpr`
- 记忆固化与演化：写回、级联、冗余检查
...
- 处理流量：PCAP 解析 -> 流量 Note -> Top-k 检索 -> 规则提案
- 沙盒循环：`max 3` 次失败分析与重生成
...
- `Init`：上传初始规则文件（`.rules` / `.txt`）
- `Process`：上传 `pcap`（攻击/正常）或直接粘贴 `traffic_text`
